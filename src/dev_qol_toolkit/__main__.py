...
from . import json_utils, file_utils, string_utils, config_utils, system_utils, security_utils, time_utils, validation_utils
...
def add_file_commands(subparsers: argparse._SubParsersAction) -> None:
    ...
    find_parser.add_argument("--root", type=Path, default=Path("."), help="Root directory to search")
...
def add_string_commands(subparsers: argparse._SubParsersAction) -> None:
    ...
    convert_parser.add_argument("--to", choices=["snake", "camel", "pascal", "kebab"], 
                               required=True, help="Target case format")
    ...
    validate_parser.add_argument("--type", choices=["email", "url"], 
                                required=True, help="Validation type")
...
def add_config_commands(subparsers: argparse._SubParsersAction) -> None:
    ...
    load_parser.add_argument("--format", choices=["json", "yaml", "toml", "env"], 
...
def add_security_commands(subparsers: argparse._SubParsersAction) -> None:
    ...
    hash_parser.add_argument("--algorithm", choices=["sha256", "sha512", "md5"], 
    ...
    uuid_parser = security_sub.add_parser("generate-uuid", help="Generate UUID")
    ...
    if args.config_cmd == "load":
        try:
            if args.format == "json" or (not args.format and args.path.suffix == ".json"):
                data = json_utils.load_json(args.path)
            elif args.format == "yaml" or (not args.format and args.path.suffix in [".yaml", ".yml"]):
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        prog="dev-qol-toolkit", 
        description="Comprehensive developer quality-of-life utilities"
    )
    subparsers = parser.add_subparsers(dest="command", required=True, help="Available command categories")
