...
class TestValidator:
    ...
    def test_validate_exception_handling(self):
        ...
        validator = Validator()
        validator.add_rule(
            "field", lambda x: x.upper(), "Should not fail"
        )  # Will raise AttributeError for non-strings
    ...
    def test_multiple_rules_same_field(self):
        ...
        validator.add_rule(
            "password", lambda x: any(c.isupper() for c in str(x)), "Password needs uppercase"
        )
