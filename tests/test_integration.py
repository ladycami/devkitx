...
class TestModuleIntegration:
    ...
    def test_json_file_workflow(self, tmp_path: Path):
        ...
        original_data = {
            ...
            "config": {"debug": True, "max_connections": 100, "timeout_seconds": 30.5},
        }
...
class TestPerformanceBenchmarks:
    ...
    def test_json_processing_performance(self, tmp_path: Path):
        ...
        large_data = {
            ...
            "config": {"settings": {f"key_{i}": f"value_{i}" for i in range(100)}},
        }
    ...
    def test_validation_performance(self):
        ...
        start_time = time.perf_counter()
        range_results = [validation_utils.validate_range(val, 0, 1000) for val in test_values]
        range_time = time.perf_counter() - start_time

        # Benchmark length validation
        start_time = time.perf_counter()
        length_results = [validation_utils.validate_length(s, 10, 50) for s in test_strings]
...
class TestErrorHandlingIntegration:
    ...
    def test_validation_error_integration(self):
        ...
        invalid_data = {"nested": {"deeply": {"invalid": object()}}}  # Non-serializable object
